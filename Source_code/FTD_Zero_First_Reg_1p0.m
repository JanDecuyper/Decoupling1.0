function [W,V,H,G,errorTot,argout]= FTD_Zero_First_Reg_1p0(tensor, WVHGinit, x,F,options)
...
%     - lambda: hyperparameter used as weight for the penalty term
...
lambda = zeros(1,r);
for i=1:r
    lambda(i) = lambda_or/(r*N); % potentially select a lambda per branch
end

% Broadcast lambda (in case of branch specific lambda)
lambda_broad = diag(fVec(repmat(lambda,N,1)));
lambda_broad_V = fVec(repmat(lambda,N,1));
...
for ALSit = 1:maxit
    ...
    problem.objective = @(v) CostV(v,n,tensor,W,G,x,lambda_broad_V,alphaJ,Dlevel);
    problem.x0 = v0;
    ...
    % To solve: || J3 - [KF; KJ; KReg] vec(G)||_F 
    ...
    K = [sqrt(alphaF)*KF;sqrt(alphaJ)*KJ;sqrt(lambda_broad)*KReg];
    
    % Solve for G
    vecG = lsqminnorm(K,[sqrt(alphaF)*fVec(F);sqrt(alphaJ)*fVec(T3);zeros(N*r,1)]);
    ...
    K = [sqrt(alphaF)*G; sqrt(alphaJ)*kr(HC,V)];
    W = (K \ [sqrt(alphaF)*F;sqrt(alphaJ)*T1'])';

    % 4) report cost function value & visualise result
    
    % update terms to be able to compute the error on the objectives
    % separately
    ...
    % Compute error terms separately
    FerrorNorm = frob(sqrt(alphaF)*fVec(F)-sqrt(alphaF)*KF*fVec(G));
    JerrorNorm = frob(sqrt(alphaJ)*fVec(T3)-sqrt(alphaJ)*KJ*fVec(G));
    RegerrorNorm =  frob(zeros(N*r,1)-sqrt(lambda_broad)*KReg*fVec(G));
    
    errorTot(ALSit) = sqrt(FerrorNorm.^2 + JerrorNorm.^2 + RegerrorNorm.^2);
    
    disp(['Objective terms - 0: ' num2str(FerrorNorm) ' - 1: ' num2str(JerrorNorm) ' - Reg: ' num2str(RegerrorNorm) ' - Tot: ' num2str(errorTot(ALSit))])
...
function CV = CostV(v,n,tensor,W,G,x,lambda_broad,alphaJ,Dlevel)
...
% LM error series %
CV = [sqrt(alphaJ)*fVec(T21);sqrt(lambda_broad).*fVec(HL)] - [sqrt(alphaJ)*fVec(V*(kr(H3,W).'));sqrt(lambda_broad).*fVec(HR)]; % add regularisation term that penalises difference between the left and right derivative
