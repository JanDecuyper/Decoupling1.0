...
Lambdas = [1e-12,1e-8, 1e-4,1,1e2,1e4, 1e8, 1e12];
%Lambdas = [1e2,1e4];
...
for r=1:5
    ...
    for l=1:length(Lambdas)
        ...
        [W01,V01,H01,G01,errorT301,argout01]= FTD_Zero_First_Reg_1p0(J, WVHGinit, x,F,options);
        options.alphaF = 0;
        % options.lambda = lambda/3;
        [W1,V1,H1,G1,errorT31,argout1]= FTD_Zero_First_Reg_1p0(J, WVHGinit, x,F,options);
       ...
       rel_err_NN_val = rms(yVal(Ntran:end)-yValNN(Ntran:end))/rms(yVal(Ntran:end)-mean(yVal(Ntran:end)))
        ...
        %% Save results
        % r lambda rel_err_01 relerr_1_DC rel_err_modeld01 rel_err_modeld1
        % maxLipschitz
        store_results = [store_results; r lambda rel_err_01 rel_err_1_DC rel_err_modeld01 rel_err_modeld1 max(max(abs(H01))) max(max(abs(H1)))];
        store_val = [store_val; r lambda rel_err_modeld01_val rel_err_modeld1_val rel_err_NN_val];
        name = ['Ft_nx15_r_' num2str(r) '_lambda_' num2str(lambda)];
        mkdir(name)
        cd(name)
        saveAllFigures('fig')
        close all
